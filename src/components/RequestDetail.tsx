...
const RequestDetail = () => {
  ...
  const { requestId, binPath } = useParams<{
    requestId: string;
    binPath: string;
  }>();
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      ...
      try {
        const fetchedRequest = await requestService.getRequestById(
          binPath,
          requestId
        );
        console.log(
          `Fetched request with id ${requestId}, first one is: ${JSON.stringify(
            fetchedRequest
          )}`
        );
